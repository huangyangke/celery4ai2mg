...
from projects.example.engine import engine_func 
task_name = cyoperate.update_celery_task(engine_func, bind=True) 
...
def func_call(item: Item):
    task = cyoperate.send_task(task_name=task_name, args=[item.image_url,]) # 此方法会将任务放到队列中
